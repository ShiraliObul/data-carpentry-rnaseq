...
raw_cts_long %>%
  ...
  ggplot(aes(factor(minute), log10(cts + 1), colour = strain)) + 
  geom_boxplot() + 
...
```{r, echo=FALSE}
sample_pca <- prcomp(t(trans_cts[, -1]), scale = TRUE)
```
...
You will first need to _join_ your `pc_scores` table with the `sample_info` table.
Then, _colour_ the points according to "minute" and their _shape_ according to "strain".
