...
(similar to histograms) for our expression data found in the `norm_cts` object.
...
1. Convert our table of counts (`norm_cts_tbl`) to "long" format, i.e. with 3 
...
norm_cts_corr <- cor(norm_cts, method = "spearman")

# Visualise the correlations between the first 5 samples
norm_cts_corr[1:5, 1:5]
```
