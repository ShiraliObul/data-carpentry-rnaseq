...
to use our original matrix of counts stored in `norm_cts`. However, we do not want 
...
```{r}
gene_dist <- norm_cts[candidate_genes, ] %>% 
  t() %>% 
  scale() %>% 
  t() %>% 
  dist()
...
```{r}
norm_cts_cluster <- norm_cts_mean %>% 
  inner_join(gene_cluster, by = "gene")

head(norm_cts_cluster)
```
