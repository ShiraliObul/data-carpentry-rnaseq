...
That's a huge dimensionality reduction from `r nrow(norm_cts)` variables (genes) 
...
You can easily produce such a plot using the `autoplot()` function by adding the 
options `loadings = TRUE, loadings.label = TRUE`:
...
on our data (and probably crash your computer!).
...
Note that in the plot above:

* we used the autoplot function to make the main PCA plot as before
* but both the `geom_segment()` and `geom_text()` layers use a different dataset, 
which we specifically included _inside the geometry_ with `data = gene_loadings` 

This is still a bit crowded, but you can play around with these commands to 
improve the visualisation (e.g. restrict the number of genes even more). Another 
possibility is to use the `geom_text_repel()` from the [`ggrepel`](https://cran.r-project.org/web/packages/ggrepel/vignettes/ggrepel.html) 
package.
