...
* Book chapter from Roger Peng's _Exploratory Data Analysis with R_:
    * [Dimension Reduction](https://bookdown.org/rdpeng/exdata/dimension-reduction.html)
...
[gene clustering lesson](05_rnaseq_clustering.html).
...
library(patchwork)
pca_plot + coord_fixed(ratio = 0.4) + labs(tag = "A", title = "PC scores") +
  loadings_plot + coord_fixed(ratio = 0.4) + labs(tag = "B", title = "PC loadings") +
  plot_layout(ncol = 2)
```

Notice how we've forced our PC1 axis to be around twice as long as the PC2 
axis. This is because the variance explained by PC1 is about twice 
