...
Also see these two videos from StatQuest, by Josh Starmer (or just watch all of them!): 
...
[Link to exercise](00_exercises.html#31_examine_prcomp()_output)
...
[Link to exercise](00_exercises.html#32_annotating_pc_plot)
...
Another important question to ask after we do a PCA is how many PCs we have and 
...
Using the `patchwork` package, we put these two plots side by side to get a 
more complete and interpretable view of our PCA analysis:

```{r}
library(patchwork)
pca_plot + labs(tag = "A", title = "PC scores") +
  loadings_plot + labs(tag = "B", title = "PC loadings") +
  plot_layout(ncol = 2)
```



## Shortcut for a quick visualisation
...
have thousands of genes, so the plot would be un-interpretable!
