...
does the centering but not the scaling. In the code above, we changed this 
default behaviour to ensure gene expression data was scaled. 
In this case, because we are using the transformed data, this is not too much 
of an issue, but try re-running the PCA with the default options to see how it changes it. 
...
tidy(sample_pca, matrix = "variables")
```
...
Using the [`cowplot`](https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html) 
package, we put these two plots side by side to get a 
more complete and interpretable view of our PCA analysis:

```{r, fig.width=8, fig.height=3}
# note that cowplot changes the default ggplot2 theme to a cleaner look
library(cowplot)
